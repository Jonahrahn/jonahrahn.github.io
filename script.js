...
// ---------- POS playground: live parsing via Compromise --------------------
const playgroundInput = document.getElementById('playground-input');
const playgroundTokens = document.getElementById('playground-tokens');
const playgroundStats = document.getElementById('playground-stats');

if (playgroundInput && playgroundTokens && playgroundStats) {
  // Compact map: keep the most diagnostic tag for each term.
  const TAG_PRIORITY = [
    'Pronoun',
    'Determiner',
    'Preposition',
  ...
  function render() {
    ...
    if (!text) {
      playgroundTokens.innerHTML = '<span class="playground__hint">type something above</span>';
    ...
    const html = terms
      .map((t) => {
        const tags = (t.terms && t.terms[0] && t.terms[0].tags) || [];
        if (!tags.length) return `<span class="ptok"><span class="ptok__word">${t.text}</span></span>`;
        const tag = primaryTag(tags);
        const short = SHORT[tag] || tag.slice(0, 4).toUpperCase();
        const full = tags.join(' · ');
        return `<span class="ptok" title="${full}">
          <span class="ptok__tag">${short}</span>
          <span class="ptok__word">${t.text}</span>
        </span>`;
      })
      .join(' ');
    playgroundTokens.innerHTML = html;
    ...
    for (const t of terms) {
      ...
      const short = SHORT[primary] || primary.slice(0, 4).toUpperCase();
      tagCounts[short] = (tagCounts[short] || 0) + 1;
    }
    ...
    playgroundStats.innerHTML = stats
      .map((s) => `<span class="pstat">${s}</span>`)
      .join('');
  }
  ...
  // First render (after compromise has loaded via defer)
